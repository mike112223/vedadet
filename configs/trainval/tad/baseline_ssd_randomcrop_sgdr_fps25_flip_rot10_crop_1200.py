...
data = dict(
    ...
    train=dict(
        ...
        img_prefix=data_root + 'images_25fps_resize_112_112/val',
        fps=fps,
        pipeline=[
            dict(
                typename='LoadAnnotations'),
            dict(typename='VideoRandomCropBeforeLoading',
                 window_size=window_size),
            dict(typename='LoadVideoFromPatch',
                 to_float32=True),
            dict(
                typename='RandomCrop3d',
                crop_size=(96, 96)),
            dict(
                typename='Rotate3d',
                max_angle=10),
    ...
    val=dict(
        ...
        img_prefix=data_root + 'images_25fps_resize_112_112/test',
        fps=fps,
        pipeline=[
...
hooks = [
    ...
    dict(
        ...
        periods=[100] * num_restarts,
        restart_weights=[1] * num_restarts,
        warmup='linear',
        warmup_iters=500,
        warmup_ratio=1e-1,
        min_lr_ratio=1e-2),
...
# 5. work modes
modes = ['train']
max_epochs = 100 * num_restarts

# 6. checkpoint
weights = dict(
   filepath='workdir/baseline_ssd_randomcrop_sgdr_fps25_flip_rot10_crop/epoch_600_weights.pth')
optimizer = dict(filepath='workdir/baseline_ssd_randomcrop_sgdr_fps25_flip_rot10_crop/epoch_600_optim.pth')
meta = dict(filepath='workdir/baseline_ssd_randomcrop_sgdr_fps25_flip_rot10_crop/epoch_600_meta.pth')
