...
class TemporalBBoxBaseAnchor:
    ...
    def _single_level_base_anchors(base_size,
        ...
        t = base_size
        if center is None:
            t_center = center_offset * t
        else:
            t_center = center

        t_ratios = torch.sqrt(ratios)
        if scale_major:
            ts = (t * t_ratios[:, None] * scales[None, :]).view(-1)
        else:
            ts = (t * scales[:, None] * t_ratios[None, :]).view(-1)

        # use float anchor and the anchor's center is aligned with the
        # pixel center
        base_anchors = [t_center - 0.5 * ts, t_center + 0.5 * ts]
