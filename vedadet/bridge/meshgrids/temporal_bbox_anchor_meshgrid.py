...
class TemporalBBoxAnchorMeshGrid(BaseMeshGrid):
    ...
    def _single_level_anchor_mesh(self, base_anchors, featmap_size, stride,
        ...
        shift_t = torch.arange(0, feat_t, device=device) * stride
        shift_t = shift_t.type_as(base_anchors)
        # first feat_w elements correspond to the first row of shifts
        # add A anchors (1, A, 2) to K shifts (K, 1, 2) to get
        # shifted anchors (K, A, 2), reshape to (K*A, 2)

        all_anchors = base_anchors[None, :, :] + shift_t[:, None, None]
    ...
    def valid_flags(self, featmap_sizes, pad_shape, device='cuda'):
        ...
        Args:
            featmap_sizes (list(tuple)): List of feature msap sizes in
        ...
        for i in range(self.num_levels):
            ...
            feat_t = featmap_sizes[i]
            t = pad_shape[0]

            valid_feat_t = min(int(np.ceil(t / anchor_stride)), feat_t)
            flags = self._single_level_valid_flags(
                feat_t, valid_feat_t, self.num_base_anchors[i],
                device=device)
    ...
    def _single_level_valid_flags(self,
        ...
        feat_t = featmap_size
        valid_t = valid_size
        assert valid_t <= feat_t
        valid = torch.zeros(feat_t, dtype=torch.bool, device=device)
        valid[:valid_t] = 1
