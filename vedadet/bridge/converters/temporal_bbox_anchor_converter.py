...
from vedadet.misc.bbox import build_bbox_coder
from .base_converter import BaseConverter


@registry.register_module('converter')
class TemporalBBoxAnchorConverter(BaseConverter):
    ...
    def get_bboxes(self, mlvl_anchors, img_metas, cls_scores, bbox_preds):
        ...
        assert len(img_metas) == 1 and len(mlvl_anchors[0]) == 1, (
            'only support one video inference')
        num_levels = len(cls_scores)
        num_clips = cls_scores[0].shape[0]

        result_list = []
        for clip_id in range(num_clips):
            cls_score_list = [
                cls_scores[i][clip_id].detach() for i in range(num_levels)
            ]
            bbox_pred_list = [
                bbox_preds[i][clip_id].detach() for i in range(num_levels)
            ]
            # TODO: hard code. 0 for anchor_list, 1 for valid_flag_list
            anchors = mlvl_anchors[0][0]
    ...
    def _get_bboxes_single(self,
        ...
        window_size = img_metas['window_size']
        ...
        for cls_score, bbox_pred, anchors in zip(cls_score_list,
            ...
            bboxes = self.bbox_coder.decode(
                anchors, bbox_pred, max_shape=window_size)
