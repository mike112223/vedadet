...
class ResNet3dCpNet1(nn.Module):
    """ResNet 3d backbone.
    ...
    def __init__(self,
        ...
        self.connect_layers = []
        assert self.inplanes % temporal_out_channels == 0
        num_layers = int(math.log2(self.inplanes // temporal_out_channels))
        in_channels = self.inplanes
        for i in range(num_layers):
            out_channels = in_channels // 2
            self.connect_layers.append(
                ConvModule(
                    in_channels,
                    out_channels,
                    kernel_size=3,
        ...
        self.connect_layers = nn.Sequential(*self.connect_layers)

        self.cp_layers = []
        assert in_channels == temporal_out_channels
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.connect_layers):
            x = layer(x)
