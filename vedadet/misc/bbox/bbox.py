...
def bbox_overlaps1d(bboxes1, bboxes2, mode='iou', is_aligned=False, eps=1e-6):
    """Calculate overlap between two set of bboxes.
    If ``is_aligned`` is ``False``, then calculate the ious between each bbox
    of bboxes1 and bboxes2, otherwise the ious between each aligned pair of
    bboxes1 and bboxes2.
    ...
    if is_aligned:
        l = torch.max(bboxes1[:, 0], bboxes2[:, 0])  # [rows]
        r = torch.min(bboxes1[:, 1], bboxes2[:, 1])  # [rows]

        overlap = (r - l).clamp(min=0)  # [rows]
        ...
        l = torch.max(bboxes1[:, None, 0], bboxes2[:, 0])  # [rows, cols]
        r = torch.min(bboxes1[:, None, 1], bboxes2[:, 1])  # [rows, cols]

        overlap = (r - l).clamp(min=0)  # [rows, cols]
        area1 = bboxes1[:, 1] - bboxes1[:, 0]
...
def pseudo_bbox(bboxes, mode='torch'):
    """Map temporal bbox to standard detection bboxes"""

    num_bboxes = bboxes.shape[0]

    if mode == 'torch':
        pseudo_y1 = bboxes.new_full((num_bboxes, ), 0.)
        pseudo_y2 = bboxes.new_full((num_bboxes, ), 1.)
        bboxes = torch.stack(
            [bboxes[:, 0], pseudo_y1, bboxes[:, 1], pseudo_y2], dim=-1)
    elif mode == 'numpy':
        pseudo_y1 = np.zeros(num_bboxes)
        pseudo_y2 = np.ones(num_bboxes)
        bboxes = np.stack(
            [bboxes[:, 0], pseudo_y1, bboxes[:, 1], pseudo_y2], axis=-1)
    else:
        raise 'not supported'
